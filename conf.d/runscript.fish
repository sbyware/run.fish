function _run_install --on-event run_install
    echo "[run installer] setting up run variables"
    ...
    echo "[run installer] installing run v$run__version"
    echo "[run installer] creating run directory"
    mkdir -p $run__script_dir
    touch $run__history_file
    touch $run__log_file
    echo "[run installer] copying scripts to $run__dir"
    cp -r (pwd)"/scripts" $run__dir
    echo "[run installer] installed run v$run__version"
end

function _run_uninstall --on-event run_uninstall
    echo "[run uninstaller] uninstalling run v$run__version"
    echo "[run uninstaller] removing run directory"
    rm -rf $run__dir
    echo "[run uninstaller] removing run variables"
    ...
    echo "[run uninstaller] uninstalled run v$run__version"
    set -e run__version
end